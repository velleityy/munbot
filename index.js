...
const helpEmbed = new Discord.RichEmbed()
	.setAuthor("TBMUN Bot's Commands")
	...
	.addField("See results of a poll and close the voting", `\`${config.prefix} end ID\`, where ID is the poll id which
...
const examplesEmbed = new Discord.RichEmbed()
	.setAuthor("Examples of TBMUN Bot's commands")
	.addField("Speakers List", `\`${config.prefix} "GSL" "Yes"\``)
	.addField("Y/N Poll", `\`${config.prefix} "MC - 10 to 1 - Indonesia - Possible Solutions"\``)
	.addField("Complex poll", `\`${config.prefix} "Most Likely to Become a Diplomat" "Elaine" "Prabu" "Arya" "Nayfa"\``)
	.addField("Timed poll", `\`${config.prefix} time=6h "Call tonight?"\``)
...
client.on("message", async (msg) => {
	if (msg.content.startsWith(config.prefix) && !msg.author.bot) {
		...
		if (msg.channel.type === "text" || msg.channel.type === "news") {
			...
			let roleid = -1;
			try {
				role = await msg.guild.roles.find((r) => r.name === "Chairs" || "Secretariat");
			...
			if (!(msg.member.hasPermission("ADMINISTRATOR") || msg.member.roles.has(roleid))) {
				msg.reply("sorry delegate, you do not have permission to do that. Try contacting your chairs or the tech team.");
